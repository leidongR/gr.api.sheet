...
test("Error - no fields output", async () => {
  const response = await request(app.callback()).get(
    `/sheets/3584589747?$deselect=id&$select=id`
  );
  expect(response.status).toBe(403);
