...
test('Get sheet - = - not found', async () => {
    const response = await request(app.callback()).get('/sheets/3584589747?day=20220215');
    expect(response.status).toBe(200);
...
test('Get sheet - =', async () => {
    const day = 20220221;
    const response = await request(app.callback()).get(`/sheets/3584589747?day=20220221`);
    expect(response.status).toBe(200);
...
test('Get sheet - = - string', async () => {
    const day = 20220221;
    const response = await request(app.callback()).get(`/sheets/3584589747?Description=`);
    expect(response.status).toBe(200);
...
test('Get sheet - $lt, $gt', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?day$gt=20211231&day$lt=20230101`);
    expect(response.status).toBe(200);
...
test('Get sheet - $lt, $gt - string', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?holiday$gt=A&holiday$lt=F`);
    expect(response.status).toBe(200);
...
test('Get sheet - $lte, $gte', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?day$gte=20220117&day$lte=20220415`);
    expect(response.status).toBe(200);
...
test('Get sheet - $lte, $gte - string', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?holiday$gte=Christmas&holiday$lte=Independence Day`);
    expect(response.status).toBe(200);
    ...
    expect(result.count).toBe(result.data.length);
    (result.data as any[]).forEach(item => {
        expect((item.Holiday as string).localeCompare('Christmas')).toBeGreaterThan(-1);
        expect((item.Holiday as string).localeCompare('Independence Day')).toBeLessThan(1);
    });
});

test('Get sheet - $in', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?day$in=20220117&day$in=20220415&Exchange$in=NYSE`);
    expect(response.status).toBe(200);
...
test('Get sheet - $in - string', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?holiday$in=Washington%E2%80%99s%20Birthday&holiday$in=Thanksgiving Day`);
    expect(response.status).toBe(200);
    ...
    expect(result.count).toBe(result.data.length);
    (result.data as any[]).forEach(item => {
        expect(["Washington’s Birthday", "Thanksgiving Day"]).toContain(item.Holiday)
    });
});

test('Get sheet - $ne - number', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?day$gt=20211231&day$lt=20230101&day$ne=20220117&day$ne=20220415`);
    expect(response.status).toBe(200);
...
test('Get sheet - $ne - string', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?Exchange$ne=NYSE`);
    expect(response.status).toBe(200);
...
test('Get sheet - $like', async () => {
    let response = await request(app.callback()).get(`/sheets/3584589747?day$like=2021`);
    expect(response.status).toBe(403);


    response = await request(app.callback()).get(`/sheets/3584589747?holiday$like=New`);
    expect(response.status).toBe(200);
...
test('Get sheet - $select + $limit', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?$select=day&$select=holiday&$select=exchange&$limit=2`);
    expect(response.status).toBe(200);
...
test('Get sheet - $deselect + $limit', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?$deselect=id&$deselect=description&$limit=2`);
    expect(response.status).toBe(200);
...
test('Get sheet - $skip + $limit', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?$skip=2&$limit=2`);
    expect(response.status).toBe(200);
...
test('Get sheet - $sort', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?$sort=Holiday&$sort=-day`);
    expect(response.status).toBe(200);
    ...
    for (let index = 0; index < data.length - 1; index++) {
        ...
        if (compareHoliday === 0) {
            const compareDay = curr.Day > next.Day ? 1 : (curr.Day === next.Day ? 0 : -1);
...
test('Get sheet - $or', async () => {
    const response = await request(app.callback()).get(`/sheets/3584589747?$or[0]day$gte=20200101&$or[0]holiday=Christmas&$or[1]day$lt=20200101&$or[1]holiday$like=Washington`);
    expect(response.status).toBe(200);
